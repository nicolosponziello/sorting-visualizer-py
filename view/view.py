...
class View():
    ...
    def init(self, controller):
        ...
        b = tk.Button(self.frame, text="print", command= lambda: print(self.controller.getArray()))
        b.pack(side=tk.LEFT)
        b2 = tk.Button(self.frame, text="Generate", command=lambda: self.controller.generateNewArray(elem_num.get(), min_value_entry.get(), self.max_value_entry.get()))
        b2.pack(side=tk.LEFT)
        b3 = tk.Button(self.frame, text="Draw", command=lambda: self.draw_array(self.controller.getArray()))
        b3.pack(side=tk.LEFT)
        algo = tk.StringVar(self.root)
        b4 = tk.Button(self.frame, text="Sort",command=lambda: self.controller.sort(algo.get()))
